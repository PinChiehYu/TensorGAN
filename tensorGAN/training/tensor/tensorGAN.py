...
@misc.profiled_function
def normalize_1nd_moment(x, dim=1, eps=1e-8):
    return x * (x.square().mean(dim=dim, keepdim=True) + eps).rsqrt()
...
def modulated_conv1d(
    x,                          # Input tensor of shape [batch_size, in_channels, in_width].
    weight,                     # Weight tensor of shape [out_channels, in_channels, kernel_width].
    styles,                     # Modulation coefficients of shape [batch_size, in_channels].
    noise           = None,     # Optional noise tensor to add to the output activations.
    up              = 1,        # Integer upsampling factor.
    ...
    misc.assert_shape(weight, [out_channels, in_channels, kw]) # [OIk]
    misc.assert_shape(x, [batch_size, in_channels, None]) # [NIW]
    misc.assert_shape(styles, [batch_size, in_channels]) # [NI]

    # Pre-normalize inputs to avoid FP16 overflow.
    if x.dtype == torch.float16 and demodulate:
        weight = weight * (1 / np.sqrt(in_channels * kw) / weight.norm(float('inf'), dim=[1,2], keepdim=True)) # max_Ik
        styles = styles / styles.norm(float('inf'), dim=1, keepdim=True) # max_I
    ...
    dcoefs = None
    if demodulate or fused_modconv:
        w = weight.unsqueeze(0) # [NOIk]
        w = w * styles.reshape(batch_size, 1, -1, 1) # [NOIk]
    if demodulate:
        dcoefs = (w.square().sum(dim=[2,3]) + 1e-8).rsqrt() # [NO]
    if demodulate and fused_modconv:
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        memory_format = torch.channels_last if channels_last else torch.contiguous_format
        self.weight = torch.nn.Parameter(torch.randn([out_channels, in_channels, kernel_size]).to(memory_format=memory_format))
        if use_noise:
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        misc.assert_shape(x, [None, self.in_channels, in_resolution])
...
class ToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False):
        ...
        self.weight = torch.nn.Parameter(torch.randn([out_channels, in_channels, kernel_size]).to(memory_format=memory_format))
        self.bias = torch.nn.Parameter(torch.zeros([out_channels]))
        self.weight_gain = 1 / np.sqrt(in_channels * kernel_size)
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, **layer_kwargs):
        ...
        # ToRGB.
        if img is not None:
            misc.assert_shape(img, [None, self.img_channels, self.resolution // 2])
            img = torch.nn.functional.interpolate(img, scale_factor=2, mode='linear')
