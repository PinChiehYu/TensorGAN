...
@misc.profiled_function
def _upfirdn1d(x, f, up=1, down=1, padding=0, flip_filter=False, gain=1):
    ...
    # Upsample by inserting zeros.
    x = x.reshape([batch_size, num_channels, in_width, 1])
    x = torch.nn.functional.pad(x, [0, upx - 1])
    x = x.reshape([batch_size, num_channels, in_width * upx])
